...
module FastCommentsClient
  class PubSubComment < ApiModelBase
    attr_accessor :_id
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `FastCommentsClient::PubSubComment` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `FastCommentsClient::PubSubComment`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [_id, tenant_id, user_id, url_id, commenter_name, commenter_link, comment_html, comment, parent_id, votes, votes_up, votes_down, verified, avatar_src, has_images, has_links, is_by_admin, is_by_moderator, is_pinned, is_locked, display_label, rating, badges, view_count, is_deleted, is_deleted_user, is_spam, anon_user_id, feedback_ids, flag_count, domain, url, page_title, expire_at, reviewed, has_code, approved, locale, is_banned_user, group_ids, is_live, hidden, date].hash
    end
