  ...
  class PubSubVote < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `FastCommentsClient::PubSubVote` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `FastCommentsClient::PubSubVote`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @url_id_raw.nil?
        invalid_properties.push('invalid value for "url_id_raw", url_id_raw cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @url_id_raw.nil?
    ...
    # Custom attribute writer method with validation
    # @param [Object] url_id_raw Value to be assigned
    def url_id_raw=(url_id_raw)
      if url_id_raw.nil?
        fail ArgumentError, 'url_id_raw cannot be nil'
      end

      @url_id_raw = url_id_raw
    end
    ...
    def hash
      [_id, tenant_id, url_id, url_id_raw, comment_id, user_id, direction, created_at, verification_id].hash
    end
