...
module FastCommentsClient
  class UpdateTenantUserBody < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `FastCommentsClient::UpdateTenantUserBody` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `FastCommentsClient::UpdateTenantUserBody`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [username, display_name, website_url, email, sign_up_date, verified, login_count, opted_in_notifications, opted_in_tenant_notifications, hide_account_code, avatar_src, is_help_request_admin, is_account_owner, is_admin_admin, is_billing_admin, is_analytics_admin, is_customization_admin, is_manage_data_admin, is_comment_moderator_admin, is_api_admin, moderator_ids, locale, digest_email_frequency, display_label].hash
    end
